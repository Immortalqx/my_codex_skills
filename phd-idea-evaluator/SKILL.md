...
- The user has already implemented the idea and is writing the paper. Use `phd-intro-drafter`, `phd-tech-paper-template`, or `phd-benchmark-paper-template` instead.
- The user explicitly wants brainstorming of new ideas from scratch. Use plain conversation first, then draw from `references/paradigm-shift-probe.md` and `references/paradigm-examples.md` if disruptive framing becomes relevant.
- The user asks for review of an existing manuscript. Use `phd-pre-submission-reviewer`.
- The user asks to evaluate a benchmark contribution specifically. Use `phd-benchmark-paper-template` in targeted mode.
...
See: `references/paradigm-shift-probe.md` for the four probing principles (First Principles, Elephant in the Room, Technology Cycle, Hamming's Rule) and escalation rules for deeper disruptive framing.
...
Two or more yes answers means the idea has disruptive potential. Note that, and recommend revisiting `references/paradigm-shift-probe.md` plus `references/paradigm-examples.md` to deepen the disruptive framing before the user commits to a paper story.
